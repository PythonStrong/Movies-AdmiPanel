...
export default function ExclusiveVideoGet() {
  ...
  function handleDelete(id) {
    ...
    if (confirm) {
      axios.delete(`${api}api/get-cardMovies`+id)
        .then((res) => {
          alert('Movie Deleted')
          window.reload()
        });
    ...
    <div>
      ...
      <div className="flex items-center md:flex-row  flex-col">
        ...
        <div className="flex md:ml-96 flex-col gap-10 w-[100%] md:px-0 px-2 card_parent md:pt-10 ">
          ...
          {record.map((item, i) => (
            <div className="flex items-center gap-4 parent" key={i}>
              ...
              <div className="flex items-center gap-2">
                ...
                <Link href={`Edit/${item._id}`}><button className="btn btn-success text-white">Edit</button></Link>
              </div>
            </div>
          ))}
        </div>
